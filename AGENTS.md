...
이 섹션부터는 Montage 디자인 시스템 컴포넌트를 수정하거나 추가할 때 따라야 할
...
`wds-theme`에서 아직 정의되지 않은 영역(예: radius 등)은 향후 토큰화를 전제로 작업합니다.

1. 컴포넌트 폴더 안에 별도 상수를 `constants.ts`에 정의하거나 `style.ts` 상단에 모은다
2. 키 이름은 "미래에 토큰이 생겼을 때 쓸 이름"과 일치시킨다
3. 주석으로 `// TODO: wds-theme에 {토큰명} 추가 시 교체` 표시를 남긴다

예시:

```ts
// packages/wds/src/components/content-badge/constants.ts
// TODO: wds-theme에 radius 토큰 추가 시 semantic.radius.badge.* 로 교체 예정
export const CONTENT_BADGE_RADIUS = {
  xsmall: '6px',
  small: '8px',
  medium: '10px',
} as const;
...
이렇게 해두면 추후 `wds-theme`에 토큰이 추가됐을 때 이 상수 파일만 수정하면 
마이그레이션이 완료됩니다.
