...
| `domain`   | `string`                      | `'auto'`                          | `Domain` 속성. 아래 참고 |
| `path`     | `string`                      | `'/'`                             | `Path` 속성              |
...
잘못된 옵션 값은 무시되고 콘솔에 에러가 출력된 뒤 기본값으로 폴백합니다. 쿠키 옵션이 잘못되면 공통적으로 **브라우저가 쓰기를 거부하고 `document.cookie`에는 아무 흔적도 남지 않아**, 증상이 "테마가 저장되지 않음" 하나로 수렴합니다. 콘솔 에러가 유일한 단서입니다.
...
- `maxAge` — 양의 정수(초)만 허용합니다. `Max-Age`는 숫자만 받으므로 소수·음수·`NaN`(환경변수 파싱 실패 등)이 들어가면 브라우저가 속성을 통째로 무시해 **세션 쿠키**가 됩니다(브라우저를 닫으면 테마가 사라집니다). `0`은 쓰는 즉시 만료라 더 나쁩니다.
- `sameSite` — `'lax' | 'strict' | 'none'`만 허용합니다. 그 외 값(대문자 `'Lax'` 등)은 `SameSite=undefined`로 직렬화되어 브라우저가 속성을 무시하고, 의도한 정책 대신 조용히 Lax로 동작합니다.
- `path` — `/`로 시작하는 절대 경로만 허용합니다. 빈 문자열이나 상대 경로는 브라우저가 거부하는 대신 **default-path**(쿠키를 쓴 페이지의 디렉터리)로 해석합니다. 그러면 사용자가 처음 들어온 경로마다 쿠키가 흩어지고, 읽을 때는 **더 깊은 Path가 이기므로** 특정 경로에서만 낡은 테마에 고착됩니다. `cookie={{ path: process.env.COOKIE_PATH }}`가 빈 값이 되는 사고가 대표적입니다.

#### `__Secure-` / `__Host-` 쿠키 이름 프리픽스

이 두 프리픽스는 명명 규칙이 아니라 **브라우저가 강제하는 계약**입니다. 조건을 만족하지 않으면 쿠키가 통째로 거부되며, 역시 에러가 없습니다. `ThemeProvider`는 속성을 자동으로 맞추고 무엇이 조정됐는지 콘솔에 보고합니다.

| 프리픽스    | 브라우저 요구 조건                          | ThemeProvider 동작                       |
| ----------- | ------------------------------------------- | ---------------------------------------- |
| `__Secure-` | `Secure` 필수                               | `secure`를 강제로 `true`                 |
| `__Host-`   | `Secure` 필수, `Path=/` 고정, `Domain` 금지 | `secure=true`, `path='/'`, `domain` 제거 |

> **`__Host-`를 쓰면 서브도메인 공유가 불가능합니다.** `Domain` 속성 자체가 금지되므로 host-only 쿠키가 됩니다. 공유가 필요 없다는 확신이 있을 때만 쓰고, 그 경우 `domain: 'none'`을 함께 명시하면 경고 없이 동작합니다.

또한 `Secure` 쿠키는 보안 컨텍스트에서만 쓸 수 있습니다. HTTPS가 아닌 호스트(`localhost`는 예외)에서 `sameSite: 'none'`이나 위 프리픽스를 쓰면 쓰기가 조용히 실패하므로, 이 조합도 콘솔에 보고합니다.

#### 서브도메인 간 테마 공유 (기본 동작)

`cookie.domain`의 기본값은 `'auto'`입니다. **별도 설정 없이 같은 루트 도메인의 앱들이 테마를 공유합니다.**

```tsx
// www.wanted.co.kr / help.wanted.co.kr / id.wanted.co.kr이 모두 테마를 공유
<ThemeProvider enableDarkMode />
```

`'auto'`는 현재 호스트가 실제로 쓸 수 있는 **가장 넓은 `Domain`을 런타임에 탐지**합니다. 후보를 넓은 쪽부터 좁혀가며 테스트 쿠키를 써 보고 되읽어, 브라우저가 받아준 첫 값을 채택합니다.

| 호스트                            | 탐지 결과                                               |
| --------------------------------- | ------------------------------------------------------- |
| `help.wanted.co.kr`               | `.wanted.co.kr` (`.co.kr`은 public suffix라 거부됨)     |
| `example.com`                     | `.example.com`                                          |
| `user.github.io`                  | `.user.github.io` (`.github.io`가 public suffix인 경우) |
| `localhost`, `127.0.0.1`, `[::1]` | 없음 → host-only                                        |

이 방식은 앱마다 `domain` 문자열을 손으로 맞추게 하지 않는 것이 목적입니다. 설정이 엇갈리면 이름이 같은 쿠키가 서로 다른 스코프로 공존해 읽기에서 서로를 가리는데(아래 참고), 탐지로 통일하면 그 상태 자체가 생기지 않습니다. 잘못된 `domain`을 하드코딩해 쿠키가 조용히 저장되지 않는 사고 — 스테이징·프리뷰 도메인, IP 접속, public suffix — 도 함께 사라집니다.

공유를 원하지 않으면 `'none'`으로 명시적으로 끕니다.

```tsx
// 이 호스트에서만 읽히는 host-only 쿠키
<ThemeProvider enableDarkMode cookie={{ domain: 'none' }} />
```

`'auto'` / `'none'`이 아닌 값은 그대로 `Domain` 속성에 쓰입니다(`.wanted.co.kr` 등). 값은 반드시 등록 가능한 도메인이어야 합니다 — public suffix를 지정하면 브라우저가 쿠키를 거부합니다.

> 쿠키는 localStorage와 스코프가 완전히 같지는 않습니다 — **포트를 구분하지 않고**(localStorage는 origin 단위라 `:3000`과 `:4000`이 별도 저장소였습니다), `path` 스코프를 가지며, 해당 호스트로 가는 **모든 요청에 함께 전송**됩니다. 루트 도메인 전체로 공유되면 API·이미지·정적 자산 요청에도 붙으므로, 정적 자산 도메인의 CDN 쿠키 스트립 정책을 확인하세요.

##### 주의: 같은 루트 도메인의 앱은 `key`를 통일해야 합니다
...
증상은 "테마가 저장되지 않음"입니다 — 토글하면 화면은 바뀌는데 새로고침하면 되돌아가고, 탭을 다시 활성화해도 되돌아갑니다. Cookie Store API가 있는 브라우저에서는 더 즉각적입니다 — 토글 직후 `change` 이벤트로 쿠키를 다시 읽을 때 낡은 쿠키가 잡혀, 화면이 잠깐 바뀌었다가 곧바로 이전 테마로 되돌아갑니다. 에러나 경고는 발생하지 않습니다.

`ThemeProvider`는 `domain`이 잡힌 경우 **쿠키를 읽기 전에 같은 이름의 host-only 쿠키를 제거**하되, **제거 전에 값을 먼저 읽어 보존**합니다. 제거는 현재 URL로 쿠키를 보낼 수 있는 **모든 Path**(`/`, `/app`, `/app/settings` …)를 훑습니다 — 쿠키 삭제는 Path가 정확히 일치해야 하는데 읽기는 더 깊은 Path가 이기므로, `/`만 지우면 예전 배포가 `/app`에 남긴 쿠키가 그 아래 모든 페이지를 계속 가리기 때문입니다. 그래서 host-only로 먼저 배포한 뒤 도메인 공유로 넘어가는 전환에서도 사용자의 테마 선택은 유지됩니다.

host-only 스윕이 닿지 못하는 쿠키 — 이전 배포가 남긴 `.sub.wanted.co.kr` 같은 **다른 `Domain` 레벨**의 쿠키나 `Domain`이 붙은 **다른 Path**의 쿠키 — 에는 두 겹의 방어가 더 있습니다.

- **값이 다르면 추측하지 않습니다.** 인라인 스크립트와 `ThemeProvider` 모두 `document.cookie`에 같은 이름의 값이 **서로 다르게** 둘 이상 보이면 첫 번째 값을 취하지 않고 **저장된 값이 없는 것으로 취급**합니다(값이 같으면 그대로 사용합니다). 첫 값을 그대로 쓰면 Chrome에서는 항상 낡은 쪽이 잡혀 persist가 그 값을 새 쿠키 위에 덮어쓰고, 이후 모든 토글이 그 값으로 되돌아가는 잠김이 생기기 때문입니다.
- **스코프를 보고 읽고, 나머지는 정리합니다.** Cookie Store API를 지원하는 브라우저(Chrome, Safari 18.5+, Firefox 138+, HTTPS 컨텍스트)에서는 `cookieStore.getAll()`로 각 쿠키의 `Domain`/`Path`를 확인해 **자기 스코프의 값만 채택**하고, 다른 스코프에 남은 같은 이름의 쿠키는 마운트 시와 변경 이벤트마다 삭제합니다. 삭제는 `domain`이 잡힌 Provider만 수행하며, forced Provider(`enableDarkMode`가 꺼진 경우)와 `domain: 'none'` 앱은 다른 앱의 쿠키를 건드리지 않습니다.

결과적으로 `www.wanted.co.kr`과 `social.wanted.co.kr`처럼 기본 설정만 쓰는 앱들은 `.wanted.co.kr` 쿠키 하나를 공유하고, 한쪽에서 바꾼 테마가 다른 쪽에도 즉시 반영됩니다.

`domain` 기본값이 `'auto'`이므로 앱마다 값이 엇갈리는 상황은 기본적으로 발생하지 않습니다. 다만 다음 두 가지는 여전히 수동으로 맞춰야 합니다.

- **`key`** — 커스텀 키를 쓰는 앱과 기본 키를 쓰는 앱이 섞이면 서로 다른 쿠키를 보게 됩니다. 한 루트 도메인을 공유한다면 기본값 `montage-theme`을 그대로 두는 것을 권장합니다.
- **`path`** — Path가 다르면 더 긴 Path 쿠키가 읽기에서 우선하므로 경로마다 테마가 갈립니다. 기본값 `/`를 유지하세요.

##### `domain: 'none'`은 반드시 `key`와 함께 써야 합니다

`'none'`은 이 앱이 **공유 쿠키를 쓰지 않게** 할 뿐, 다른 앱이 만든 같은 이름의 도메인 쿠키가 **이 호스트로 전송되는 것은 막지 못합니다**. `document.cookie`로는 둘을 구분할 수 없으므로 그 값을 읽어 자기 host-only 쿠키에 복사하게 되고, 이후 host-only가 읽기에서 이겨 **영구히 낡은 값에 갇힙니다.**

실제 Chrome에서 측정한 결과입니다 — `c.example.com`이 `domain: 'none'`, 나머지가 기본값일 때:

```
a에서 dark 선택   → dark@.example.com,  dark@c.example.com   ← c가 복사본을 만듦
a에서 light 로 복귀 → light@.example.com, dark@c.example.com
                                          ↑ c 화면은 dark 그대로, 영원히
```

따라서 `'none'`을 쓰는 앱은 **`key`도 반드시 자기 것으로 지정해야 합니다.** 이름을 격리하는 것이 실제로 공유에서 빠지는 유일한 방법입니다. `'none'`인데 `key`가 기본값이면 콘솔 에러로 알립니다.

```tsx
// 이 앱만 별도 테마를 갖고 싶을 때
<ThemeProvider enableDarkMode cookie={{ domain: 'none', key: 'admin-theme' }} />
```
...
`nonce`를 빠뜨리면 CSP가 inline script를 차단하는데, 예외도 경고도 발생하지 않고 **다크 모드 사용자에게 첫 페인트가 라이트로 나오는 것**이 유일한 증상입니다(색상이 정적 스타일시트의 `[data-theme]` 변수에서 오기 때문에, 스크립트가 속성을 못 붙이면 라이트 팔레트가 그려집니다). v4의 `ThemeProvider`는 **스크립트가 실행되지 않았음을 감지해 콘솔에 에러를 남깁니다.** 서버에서 렌더된 스크립트인데 `data-theme`가 붙지 않은 경우만 보고하므로, 스크립트가 애초에 실행되지 않는 클라이언트 전용 렌더에서는 오탐이 없습니다.
