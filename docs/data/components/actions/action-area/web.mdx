...
`gradientColor` prop 을 이용하여 배경/그라데이션 색상을 원하는 색상으로 재정의할 수 있습니다. 생략하면 기존과 동일하게 `theme.semantic.surface.elevated.primary` 를 사용합니다.
...
const Demo = () => {
  return (
    <FlexBox sx={{ width: '100%' }}>
      <ActionArea background gradientColor="#FFE6EC">
        <ActionAreaButton variant="main">Main</ActionAreaButton>
      </ActionArea>
    </FlexBox>
  )
