---
title: TextArea
description: 여러 줄의 텍스트 입력이 필요한 경우 사용되는 텍스트 영역 입력 필드 컴포저블
---

## WantedTextArea (String 기반)
텍스트 영역 입력 필드 컴포저블입니다. 여러 줄의 텍스트 입력이 필요한 경우 사용되며, 버튼, 아이콘, 타이틀, 설명 등을 유연하게 조합할 수 있습니다. 내부적으로 `TextFieldValue`를 상태로 관리하며 `onValueChange`를 통해 외부에 값을 전달합니다.
...
텍스트 영역 입력 필드 컴포저블입니다. 커서, 선택 영역 등 복잡한 상태를 다룰 수 있는 `TextFieldValue`를 사용합니다.
