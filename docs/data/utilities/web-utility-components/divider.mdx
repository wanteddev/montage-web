...
<Demo code={`import { Divider, FlexBox, Typography } from '@montage-ui/core';
...
<Demo code={`import { Divider, FlexBox } from '@montage-ui/core';

const Demo = () => {
  return (
    <FlexBox flexDirection="column" gap="12px" sx={{ width: '100%' }}>
      <Divider />
      <Divider color="semantic.line.negative.primary" />
      <Divider color="semantic.surface.brand.primary" />
    </FlexBox>
  );
}
