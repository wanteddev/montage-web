...
- AccordionSummary에 leadingContent를 추가하거나 trailingContent를 교체할 때는 `AccordionSummaryContent`로 감싸주세요.
...
- leadingContent를 추가하거나 기본 trailingContent를 교체할 수 있어요.
- trailingContent가 a 태그이거나 클릭 이벤트가 있는 경우 `e.stopPropagation` 을 통해 아코디언이 열리고 닫히는 동작을 막을 수 있어요.
