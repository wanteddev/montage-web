...
- `CardThumbnail`의 leadingContent, trailingContent에 요소를 추가 할 때는 `CardThumbnailContent` 로 감싸 주세요.
  - Card와 leadingContent, trailingContent에 onClick 이벤트가 있는 경우, `e.stopPropagation()`과 `e.preventDefault()`를 추가하여 이벤트 전파를 막을 수 있습니다.
