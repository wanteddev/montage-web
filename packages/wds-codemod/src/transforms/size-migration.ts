...
const transformer = (file: FileInfo, api: API, options: Options) => {
  ...
  const listCellContentImports = [
    findImportDeclaration('ListCellContent', '@wanteddev/wds', j, root),
    findImportDeclaration('AccordionSummaryContent', '@wanteddev/wds', j, root),
  ];

  for (const targetImport of listCellContentImports) {
    if (!targetImport) continue;
    ...
    root
      ...
      .forEach((target) => {
        const heightAttribute = target.value.attributes?.find(
          (v): v is JSXAttribute =>
            v.type === 'JSXAttribute' && v.name.name === 'height',
        );
        ...
        if (heightAttribute) {
          ...
          target.value.attributes?.forEach((attr) => {
            if (attr.type === 'JSXAttribute' && attr.name.name === 'height') {
              attr.name.name = 'size';
            }
          });
        }
