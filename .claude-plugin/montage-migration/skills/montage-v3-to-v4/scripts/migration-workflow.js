export const meta = {
  ...
  description:
    'Run the 7 Montage v4 codemods strictly in sequence (never re-running a completed step), then scan for manual migration targets in parallel',
  whenToUse:
    'Invoked by the montage-v3-to-v4 skill to migrate a consumer repo from Montage (WDS) v3 to v4',
  phases: [
    { title: 'Codemods', detail: '7 v4 codemods, strictly sequential' },
...
//                   The seven codemod commits are intentionally non-building, so pre-commit
...
Paths may contain spaces or shell metacharacters. The \`git -C\` commands below already ship with the repo root quoted — keep that quoting when you copy them, and QUOTE every path you interpolate yourself (the npx target, mv/hash-object arguments, checkout pathspecs).
...
Report filesChanged for THIS step only: with autoCommit true, use the commit stat; with autoCommit false, diff against the step-5 snapshot (\`git -C ${repoRootSh} diff --stat <snapshot-hash> -- <each target>\`) — a plain \`git diff --stat\` there also counts every earlier step's uncommitted output and would report an inflated, cumulative number. Your final output is structured data for the orchestrator, not prose.`,
