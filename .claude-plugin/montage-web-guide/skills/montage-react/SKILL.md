...
컴포넌트 작성 전 **반드시** Montage 코딩 가이드라인을 먼저 확인합니다.
...
새 컴포넌트를 만들기 전, Montage에서 제공하는 컴포넌트가 있는지 **반드시** 확인합니다.
...
1. **Montage 컴포넌트 우선 사용**: 직접 구현하기 전에 Montage 컴포넌트를 최대한 활용합니다.
2. **확장 시 Montage 기반**: 커스텀이 필요한 경우에도 Montage 컴포넌트를 기반으로 확장합니다.
3. **일관성 유지**: Montage의 패턴과 API 설계를 따릅니다.

### 3. 디자인 토큰 적용

#### 3.1 토큰 목록 조회

스타일링 시 하드코딩된 값 대신 Montage 디자인 토큰을 사용합니다.
...
- 색상: `#RRGGBB` 대신 Montage 색상 토큰 사용
- 타이포그래피: Montage typography 토큰 사용
- 그림자, 테두리 등: Montage에서 정의된 토큰 사용

### 4. 아이콘 사용

아이콘이 필요한 경우 Montage 아이콘 라이브러리를 사용합니다.
...
- [ ] Montage 코딩 가이드라인을 따랐는가?
- [ ] Montage 컴포넌트를 최대한 활용했는가?
- [ ] 하드코딩된 스타일 값 대신 디자인 토큰을 사용했는가?
- [ ] 컴포넌트 옵션으로 제공되어 있는 값을 커스텀 스타일로 사용하지는 않았는가?
- [ ] Montage 아이콘을 사용했는가? (필요한 경우)
- [ ] Montage 유틸리티 함수를 활용했는가? (해당되는 경우)
